...
if os.name == "nt":
    ...
    lib = CDLL(DARKNET_DIR + "/darknet.so", RTLD_GLOBAL)
...
def detect(net, meta, image_bgr, thresh=.5, hier_thresh=.5, nms=.45, debug= False):
    """
    Performs the meat of the detection
    """
    #pylint: disable= C0321
    #im = load_image(image, 0, 0)
    custom_image_bgr = image_bgr # cv2.imread(image) # use: detect(,,imagePath,)
    ...
    if showImage:
        try:
            ...
            for detection in detections:
                ...
                if bounds[0] == float("inf") or bounds[1] == float("inf") or bounds[2] == float("inf") or bounds[3] == float("inf"):
                    continue
